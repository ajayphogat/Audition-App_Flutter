...
class OtherService {
  ...
  Future<List<JobModel>> getStudioJobs({
    ...
    try {
      ...
      print("2");
      http.Response res = await http
          .get(Uri.parse("$url/api/getStudioJob"), headers: <String, String>{
        "Content-Type": "application/json; charset=UTF-8",
    ...
    try {
      ...
      httpErrorHandel(
        ...
        onSuccess: () {
          ...
          for (int i = 0; i < jsonDecode(res.body).length; i++) {
            _appliedUser.add(jsonDecode(res.body)[i]);
          }
          // print("sss");
          navigatePop();
          navigatePush(_appliedUser, jobId);
          // Navigator.pop(context);
          // Navigator.pushNamed(context, DescriptionPage.routeName,
          //     arguments: jsonDecode(res.body));
        },
  ...
  Future<List<JobModel1>> showWorkingJobs({
    ...
    try {
      ...
      http.Response res = await http.get(
          Uri.parse("$url/api/showWorkingJobs?working=$working"),
          headers: <String, String>{
            "Content-Type": "application/json; charset=UTF-8",
            "x-auth-token": token!,
          });
      ...
      httpErrorHandel(
        ...
        res: res,
        onSuccess: () {
          for (int i = 0; i < jsonDecode(res.body)[working].length; i++) {
            workingJobs.add(
              JobModel1.fromJson(
                jsonEncode(jsonDecode(res.body)[working][i]),
              ),
            );
          }
        },
  ...
  Future<List<JobModel1>> showStudioJobs({
    ...
    try {
      ...
      http.Response res = await http.get(
          Uri.parse("$url/api/studio/showWorkingJobs?working=$working"),
          headers: <String, String>{
            "Content-Type": "application/json; charset=UTF-8",
            "x-studio-token": token!,
          });
  ...
  Future<void> artistProfileData({
    ...
    try {
      var userProvider = Provider.of<UserProvider1>(context, listen: false);
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String? token = prefs.getString("x-studio-token");
      if (token == null) {
        await prefs.setString("x-studio-token", "");
